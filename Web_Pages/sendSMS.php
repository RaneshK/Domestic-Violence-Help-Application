...
$dbConn = new mysqli("localhost", "root", "", "Users");
    ...
    // Get the message for the selected option
    $secondSQLQuery = "SELECT * FROM `Messages` WHERE `userID`= '$userID' AND `messageOption` = '$messageOption'";
    ...
    // Perform the query to get the user's contacts
    $rs = $dbConn->query($secondSQLQuery);
...
foreach ($phoneNumbers as $number){
    $numbersInTheCorrectFormat[] = substr_replace($number, "+61", 0, 1);
}
